...
class WebPushSubscriptionController extends Controller
{
    ...
    public function storeDriver(Request $request): JsonResponse
    {
        ...
        $driver = $user->restaurantDriver()->where('is_active', true)
            ->whereHas('shop', fn ($query) => $query->where('is_active', true)->where('catalog_type', 'restaurant'))
            ->firstOrFail();
        $endpoint = $data['subscription']['endpoint'];
        $endpointHash = hash('sha256', $endpoint);
        ...
        $request->session()->put('driver_web_push_endpoint_hash', $endpointHash);

        return response()->json(['registered' => true]);
    }

    public function destroyDriver(Request $request): JsonResponse
    {
        $data = $request->validate(['endpoint' => ['required', 'url:https', 'max:2048']]);
        $user = $request->user();
        abort_unless($user?->isRestaurantDriver(), 403);

        WebPushSubscription::query()
            ->where('user_id', $user->id)
            ->where('endpoint_hash', hash('sha256', $data['endpoint']))
            ->delete();
        $request->session()->forget('driver_web_push_endpoint_hash');
