...
function initMediaStorySlider(slider) {
        ...
        slider.dataset.mediaStoryReady = 'true';
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('[data-media-story]').forEach((slider) => {
        ...
        function renderProductsScatter(deptTitle) {
            ...
            track.classList.remove('is-shop-grid');
        ...
        function renderDepartmentsForCenter(centerIndex, person = null) {
            ...
            const isShopGrid = depsToRender.length > 0
                && depsToRender.every((item) => item?.kind === 'shop');
            track.classList.toggle('is-shop-grid', isShopGrid);

            const positions = isShopGrid
                ? depsToRender.map(() => ({ x: 0, y: 0, itemSize: 110 }))
                : scatterLayout(depsToRender.length, {
