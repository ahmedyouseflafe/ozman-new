...
<head>
    <title>إضافة منتج جديد - Ozman</title>
    <meta charset="UTF-8">
...
<body>
    <div class="shell">
        ...
        <main class="main">
            @include('admin.includes.header', ['title' => 'إضافة منتج جديد'])
            <div class="content">
                <header class="page-head">
                    <div>
                        <h1>إضافة منتج جديد</h1>
                        <p>أدخل بيانات المنتج، السعر، المخزون، الصور، وحالة الظهور.</p>
                    </div>
                ...
                <form class="form-shell" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <section class="form-section">
                        <div class="section-head">
                            ...
                            <div>
                                <h2>بيانات المنتج</h2>
                                <p>المتجر والفئة والاسم والوصف.</p>
                            </div>
                        </div>
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label" for="shop_id"><i class="ti ti-building-store"></i>المتجر</label>
                                <select id="shop_id" name="shop_id" required>
                                    <option value="">اختر المتجر</option>
                                    @foreach($shops as $shop)
                                        <option value="{{ $shop->id }}" @selected(old('shop_id', $selectedShopId) == $shop->id)>{{ $shop->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select id="category_id" name="category_id" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" data-shop-id="{{ $category->shop_id }}" @selected(old('category_id', $selectedCategoryId) == $category->id)>{{ $category->name }} - {{ $category->shop?->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="name"><i class="ti ti-tag"></i>اسم المنتج</label>
                                <input type="text" id="name" name="name" value="{{ old('name') }}" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="slug"><i class="ti ti-link"></i>الرابط المختصر</label>
                                <input type="text" id="slug" name="slug" value="{{ old('slug') }}" placeholder="اختياري">
                            </div>
                            <div class="form-group full">
                                <label class="form-label" for="description"><i class="ti ti-align-right"></i>الوصف</label>
                    ...
                    <section class="form-section">
                        <div class="section-head">
                            ...
                            <div>
                                ...
                                <p>الأسعار، الكمية، SKU، الباركود، والتقييم.</p>
                            </div>
                        </div>
                        <div class="form-grid">
                            <div class="form-group"><label class="form-label" for="price">السعر</label><input type="number" step="0.01" min="0" id="price" name="price" value="{{ old('price') }}" required></div>
                            <div class="form-group"><label class="form-label" for="discount_price">سعر الخصم</label><input type="number" step="0.01" min="0" id="discount_price" name="discount_price" value="{{ old('discount_price') }}"></div>
                    ...
                    <section class="form-section">
                        <div class="section-head">
                            ...
                            <div>
                                ...
                                <p>صورة رئيسية، فيديو اختياري، وصور إضافية للمنتج.</p>
                            </div>
                        </div>
                        <div class="upload-grid">
                            <label class="upload-box"><input type="file" name="main_image" accept="image/*"><span class="card-icon"><i class="ti ti-photo"></i></span><span><span class="card-title">الصورة الرئيسية</span><span class="card-sub">PNG أو JPG</span></span></label>
                            <label class="upload-box"><input type="file" name="video" accept="video/*"><span class="card-icon"><i class="ti ti-video"></i></span><span><span class="card-title">فيديو المنتج</span><span class="card-sub">MP4 أو WebM</span></span></label>
                            <label class="upload-box"><input type="file" name="images[]" accept="image/*" multiple><span class="card-icon"><i class="ti ti-library-photo"></i></span><span><span class="card-title">صور إضافية</span><span class="card-sub">يمكن اختيار أكثر من صورة</span></span></label>
                        </div>
                    </section>
                    ...
                    <section class="form-section">
                        <div class="section-head">
                            ...
                            <div>
                                <h2>حملات المنتج</h2>
                                <p>أضف أكثر من حملة للمنتج، كل حملة تكون صورة أو فيديو مع عنوان.</p>
                            </div>
                        </div>
                                        ...
                                        <div class="form-group full">
                                            <label class="upload-box">
                                                ...
                                                <span>
                                                    ...
                                                    <span class="card-sub">اختر صورة أو فيديو حسب نوع الحملة</span>
                                                </span>
                                            </label>
                                        </div>
                                    </div>
                    ...
                    <section class="form-section">
                        <div class="switch-grid">
                            <div class="switch-card">
                                ...
                                <label class="switch" for="is_featured"><input type="checkbox" id="is_featured" name="is_featured" value="1" @checked(old('is_featured'))><span class="slider"></span></label>
                            </div>
                            <div class="switch-card">
                                ...
                                <label class="switch" for="is_active"><input type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', true))><span class="slider"></span></label>
                            </div>
                        </div>
                    </section>

                    <div class="form-actions">
                        <button type="submit" class="btn btn-primary"><i class="ti ti-device-floppy"></i>حفظ المنتج</button>
